...
def train(model, train_loader, validation_loader, optimizer, loss_func, dist, edge_index, device, config):
    ...
    for epoch in range(config.num_epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            outputs, _ = model(inputs, None, None, dist, edge_index, None, None, targets, train_mask="Yes")
...
def main():
    ...
    config = ConfigLoader(os.path.abspath(os.path.join(ml_dir, "mp_pde", "mp_pde.yml"))).get_config()
